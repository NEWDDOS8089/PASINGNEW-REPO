...
TOKEN = '7211573106:AAH4xKv7QhCYb-FrmUyZyv0bpqTSr1HGzdk'
...
FORWARD_CHANNEL_ID = -1002140300579
CHANNEL_ID = -1002140300579
error_channel_id = -1002140300579
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Press Again your Command*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "":
        ...
        bot.send_message(message.chat.id, "*Dm - @danger_boy_op*", parse_mode='Markdown')
        ...
        bot.reply_to(message, "*/attack then enter ip port duration*", parse_mode='Markdown')
    elif message.text == "/owner":
        bot.reply_to(message, "*Owner - @SHUBH_K2*", parse_mode='Markdown')
    else:
